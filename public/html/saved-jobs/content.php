...
$jquery=select("saved_jobs","where pid='$pid' order by sjid desc ");
if (rows($jquery)>0) {
while ($data=records($jquery)) {
  $jid=$data['jobid'];
    ...
    ?>
    <p class="_card text-left my-5 py-5">
      You did not save any job yet. <a href="/jobs">Find New Jobs ?</a>
    </p>
    <?php
  }
