...
if (param1!="") {
  $id=param1;
  $query=select("users","where username='$id'");
  $profs=records($query);
  $uid=$profs['uid'];
  $proname=$profs['name'];
  $query2=select("profs","where uid='$uid'");
  $prof=records($query2);
  $pid=$prof['pid'];
}
else {
        ...
        <div class="_card p-4">
          ...
          <div class="verifications" id="lng">
            ...
            while ($row=records($lang)) {
              ?>
              <li id="lan<?php echo $row['lid']; ?>"> <?php echo $row['language'] ?> - <?php echo $row['level'] ?>
              <?php
            }
             ?>
          </div>
